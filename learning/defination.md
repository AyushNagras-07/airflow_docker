...
What happens if two DAG runs overlap?
        Airflow should ensure that only one DAG run per logical date loads data at a time, typically by using max_active_runs=1, combined with idempotent delete-and-insert logic